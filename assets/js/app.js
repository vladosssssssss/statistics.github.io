    ...
    html += `
      <tr style="border-bottom:1px solid rgba(255,255,255,0.1)">
        ...
        <td style="text-align:right;padding:8px">
          ${isAdditionalPayment ? `
            <button onclick="editDoplataPayment('${recordId}', '${doplataIndex}')" style="margin-right:5px;background:transparent;border:none;color:rgba(255,255,255,0.7);cursor:pointer;font-size:14px;padding:2px" title="Редактировать доплату">✏️</button>
...
function submitPaymentInline(recordId) {
  ...
  if(index !== -1){
    ...
    fetch(API, {
      ...
      flashLoader();
      sync(); // Синхронизировать немедленно
      setTimeout(() => {
        render(); // Отрисовать после синхронизации
      }, 1000);
    });
  }
...
function editDoplataPayment(recordId, doplataIndex) {
  ...
  if(index !== -1){
    ...
    fetch(API, {
      ...
      sync();
      setTimeout(() => {
        render(); // Update main view after sync
      }, 1000);
    });
  }
}
...
function updateMainChart(dim, dayStats, isAllTime, opts){
  ...
  // ── Always show monthly line chart: total payments by day of month across all selected months ──
  const labels = Array.from({length:31}, (_,i) => i+1);
  const data = labels.map(d => dayStats[d] || 0);
...
document.getElementById('addForm').onsubmit=e=>{
  ...
  const potentialEarnings=(amount*0.06).toFixed(0);
  ...
  db.push({"ID":id,"Дата":entry.date,"Сумма":entry.amount,"ФактСумма":entry.actualAmount,"Проект":entry.project,"Заработок":entry.earnings,"ФактЗаработок":entry.actualEarnings,"Комментарий":entry.comment,"Manager":mgrId});
  localStorage.setItem(DB_KEY,JSON.stringify(db));render();
  fetch(API,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(entry)}).then(()=>setTimeout(sync,1500));
