...
function populateGuestSel(){
  ...
  if(sel) sel.innerHTML='<option value="admin">Админ</option>' + acc.managers.map(m=>`<option value="${escAttr(m.id)}">${esc(m.name)}</option>`).join('');
}
...
function guestLogin(){
  ...
  if(sel&&sel.value){
    if(sel.value === 'admin'){
      // Гость-админ видит всё как админ но без прав редактирования
      setSess({role:'guest',name:'👁 Админ',id:'guest',viewMgrId:'admin', isAdminView:true});
    } else {
...
function applyRoleUI(){
  ...
  // Показываем админскую панель для админа и для гостя-админа
  const isAdminView = s.role==='admin' || (s.role==='guest' && s.isAdminView);
  document.getElementById('adminBar').className = isAdminView ? 'on' : '';
  if(isAdminView) rebuildAdminFilters();
  
  // Скрываем формы редактирования для всех гостей (включая гостя-админа)
  if(s.role==='guest'){
...
function showPaymentHistory(recordId) {
  console.log('showPaymentHistory called with recordId:', recordId);
  ...
  let html = `
    <div style="margin-bottom:15px">
      <strong>Клиент:</strong> ${s.role==='guest'?'Скрыто':esc(record["Комментарий"] || 'Без имени')}<br>
    ...
    <div style="margin-bottom:15px">
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        ...
        <button onclick="console.log('Button clicked, recordId: ${recordId}'); submitPaymentInline('${recordId}')" style="background:var(--success);color:white;padding:6px 12px;border:none;border-radius:6px;cursor:pointer;font-weight:700;font-size:12px">
          + Добавить доплату
        </button>
      </div>
    </div>
      ...
      html += `
        <tr id="doplata_${recordId}_${doplataIndex}" style="border-bottom:1px solid rgba(255,255,255,0.1)">
          ...
          <td style="text-align:right;padding:8px">
            <button id="doplata_edit_${recordId}_${doplataIndex}" onclick="startEditDoplata('${recordId}', '${doplataIndex}')" class="doplata-edit-btn" style="margin-right:5px;background:transparent;border:none;color:rgba(255,255,255,0.7);cursor:pointer;font-size:14px;padding:2px" title="Редактировать доплату">✏️</button>
            <button id="doplata_save_${recordId}_${doplataIndex}" onclick="saveEditDoplata('${recordId}', '${doplataIndex}')" class="doplata-save-btn" style="margin-right:5px;background:var(--success);border:none;color:white;cursor:pointer;font-size:12px;padding:2px 4px" title="Сохранить">✓</button>
            <button id="doplata_cancel_${recordId}_${doplataIndex}" onclick="cancelEditDoplata('${recordId}', '${doplataIndex}')" class="doplata-cancel-btn" style="margin-right:5px;background:var(--danger);border:none;color:white;cursor:pointer;font-size:12px;padding:2px 4px" title="Отменить">✗</button>
  ...
  console.log('About to set innerHTML and open modal');
  document.getElementById('paymentHistoryContent').innerHTML = html;
  console.log('innerHTML set, about to open modal');
...
function startEditDoplata(recordId, doplataIndex) {
  ...
  // Show save/cancel buttons, hide edit button using CSS classes
  document.getElementById(`doplata_edit_${recordId}_${doplataIndex}`).classList.add('hidden');
  document.getElementById(`doplata_save_${recordId}_${doplataIndex}`).classList.add('visible');
  document.getElementById(`doplata_cancel_${recordId}_${doplataIndex}`).classList.add('visible');
}
...
function cancelEditDoplata(recordId, doplataIndex) {
  ...
  // Hide save/cancel buttons, show edit button using CSS classes
  document.getElementById(`doplata_edit_${recordId}_${doplataIndex}`).classList.remove('hidden');
  document.getElementById(`doplata_save_${recordId}_${doplataIndex}`).classList.remove('visible');
  document.getElementById(`doplata_cancel_${recordId}_${doplataIndex}`).classList.remove('visible');
}
...
function submitPaymentInline(recordId) {
  console.log('submitPaymentInline called with recordId:', recordId);
  ...
  console.log('paymentDate:', paymentDate, 'paymentAmount:', paymentAmount);
...
async function addNewProject(){
  ...
  const mgrId=(s.role==='admin' || (s.role==='guest' && s.isAdminView))?getSingleSelectedAdminMgrId():s.id;
...
function renderInternal(){
  ...
  const curMgrId=s.role==='manager'?s.id:((s.role==='admin' || (s.role==='guest' && s.isAdminView))?(selMgrs.length===1?selMgrs[0]:null):null);
  ...
  else if(s.role==='guest'&&s.viewMgrId && !s.isAdminView)mgrFilter=[s.viewMgrId]; // обычный гость видит только одного менеджера
  else if((s.role==='admin' || (s.role==='guest' && s.isAdminView))&&selMgrs.length>0)mgrFilter=selMgrs; // админ и гость-админ видят всех выбранных

  let projFilter=null;
  if((s.role==='admin' || (s.role==='guest' && s.isAdminView))&&selProjs.length>0)projFilter=selProjs;

  // Payout grid
  if(s.role!=='guest'){
    const payMgrId=s.role==='manager'?s.id:((s.role==='admin' || (s.role==='guest' && s.isAdminView))?getSingleSelectedAdminMgrId():null);
    if((s.role==='admin' || (s.role==='guest' && s.isAdminView)) && !payMgrId){
  ...
  const isAllTime=viewAllTime||((s.role==='admin' || (s.role==='guest' && s.isAdminView))&&adAllTime);
  ...
  db.forEach(i=>{
    ...
    // month history (filtered)
    {
      ...
      if(mgrOk&&prOk){
        const inHistoryPeriod = (s.role==='admin' || (s.role==='guest' && s.isAdminView)) ? inAdPeriod(i["Дата"]) : iym.startsWith(monthFilter.split('-')[0]);
    ...
    let inPer;
    if(s.role==='admin' || (s.role==='guest' && s.isAdminView))inPer=inAdPeriod(i["Дата"]);
  ...
  db.filter(i=>{
    ...
    const isPay=proj.startsWith("Выплата")||proj.startsWith("Ожидание");
    let perOk;
    if(s.role==='admin' || (s.role==='guest' && s.isAdminView))perOk=inAdPeriod(i["Дата"]);
    ...
    tbody.innerHTML+=`<tr>
      ...
      <td style="opacity:.45;font-size:11px">${s.role==='guest'?'Скрыто':esc(i["Комментарий"]||'')}</td>
  ...
  if (managerBtn) {
    managerBtn.style.display = (s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) ? 'inline-block' : 'none';
  }
  ...
  // Analytics
  if(s.role==='admin' || (s.role==='guest' && s.isAdminView)){
...
function updateMainChart(dim, dayStats, isAllTime, earnMonthStats, maxDayToShow, opts){
  ...
  if (showMonthsChart) {
    ...
    // Filter data for selected period if not all-time
    let filteredDayStats = {...dayStats};
    if (!isAllTime) {
      // Use appropriate filter based on role
      const s = getSess();
      if (s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) {
        // Admin uses admin period filter
        if (!adAllTime) {
          filteredDayStats = {};
          db.forEach(item => {
        ...
        // Manager and guest use regular month filter
        const monthFilter = document.getElementById('filterMonth')?.value;
        if (monthFilter) {
          filteredDayStats = {};
          db.forEach(item => {
    ...
    if (showMonthsChart) {
      ...
      const dayData = db.filter(item => {
        ...
        if (!isAllTime) {
          ...
          if (s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) {
            // Admin uses admin period filter
            monthMatches = adAllTime || inAdPeriod(item["Дата"]);
          } else {
            // Manager and guest use regular month filter
            monthMatches = !monthFilter || itemMonth === monthFilter;
          }
        }
...
function renderExtendedStats(mgrFilter, projFilter, acc) {
  ...
  // Apply time period filter based on role
  const monthFilter = document.getElementById('filterMonth')?.value;
  const isAllTime = viewAllTime;
  
  if (session.role === 'admin' || (session.role === 'guest' && session.isAdminView)) {
    // Admin uses admin period filter
    if (!adAllTime) {
      filteredData = filteredData.filter(item => inAdPeriod(item["Дата"]));
    }
  } else {
    // Manager and guest use regular month filter
  ...
  const currentSession = getSess();
  
  if (currentSession.role === 'admin' || (currentSession.role === 'guest' && currentSession.isAdminView)) {
    // Admin uses admin period filters
    if (adAllTime) {
      // All time mode - use current month
      totalDaysInMonth = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
    } else {
      // Use admin from/to period - calculate days in the selected period
      const from = document.getElementById('adFrom').value;
      const to = document.getElementById('adTo').value;
      
      if (from && to) {
        // For multi-month periods, use the target month (to)
        const [year, month] = to.split('-');
        totalDaysInMonth = new Date(parseInt(year), parseInt(month), 0).getDate();
      } else {
    ...
    if (!isAllTime && monthFilter) {
      ...
      // For all-time mode, use current month
      totalDaysInMonth = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
    } else {
...
function showProjectChart() {
  ...
  if (s.role === 'manager') {
    mgrFilter = [s.id];
  } else if (s.role === 'guest' && s.viewMgrId && !s.isAdminView) {
    mgrFilter = [s.viewMgrId];
  } else if ((s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) && selMgrs.length > 0) {
    mgrFilter = selMgrs;
  }
  ...
  let projFilter = null;
  if ((s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) && selProjs.length > 0) {
    projFilter = selProjs;
  }
...
function exportToCSV(data) {
  ...
  const csvContent = [
    ...
    ...data.map(item => [
      ...
      item["Сумма"] || 0,
      item["Заработок"] || 0,
      `"${String(isGuest ? 'Скрыто' : (item["Комментарий"] || '')).replace(/"/g, '""')}"`
    ].join(','))
  ].join('\n');
...
function exportToJSON(data) {
  ...
  const exportData = {
    ...
    data: data.map(item => ({
      ...
      comment: isGuest ? 'Скрыто' : (item["Комментарий"] || '')
    })),
    summary: {
...
function renderAnalytics(mgrFilter, projFilter, acc){
  ...
  // Collect months in current period + active filters
  const moSet = new Set();
  const monthFilter = document.getElementById('filterMonth')?.value;
  const isAllTime = viewAllTime;
  const s = getSess();
  ...
  db.forEach(i=>{
    ...
    // Apply time period filter based on role and mode
    let includeRecord = false;
    if (s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) {
      // Admin uses admin period filter
      includeRecord = inAdPeriod(i["Дата"]);
    } else {
      // Manager and guest use regular month filter
      if (isAllTime) {
        includeRecord = true; // All time mode includes everything
      } else if (monthFilter) {
        includeRecord = m === monthFilter;
      } else {
        includeRecord = true; // Default to include if no filter
      }
    }
  ...
  db.forEach(i=>{
    ...
    if (s.role === 'admin' || (s.role === 'guest' && s.isAdminView)) {
      ...
      if (isAllTime) {
        ...
        includeRecord = ym(i["Дата"]) === monthFilter;
      } else {
        includeRecord = true; // Default to include if no filter
      }
    }
